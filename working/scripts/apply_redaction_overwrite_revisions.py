#!/usr/bin/env python
"""
Promote overwrite-reverted revisions when the only real change is redaction markup.

This repairs pages in Category:覆盖版本未检查的裁判文书 that were reverted after an
import whose content is otherwise identical to the current page, except that the
import uses {{PRC-redact|N}} where the old page has literal × markers. Pages with
any non-redaction difference are left in the category for manual review.
"""
...
DEFAULT_LOCK_PATH = PROJECT_ROOT / "working" / "output" / "apply_redaction_overwrite_revisions.lock"
IMPORT_SUMMARY_PREFIX = "Imported from 裁判文书网"
REVERT_SUMMARY = "回退覆盖导入版本并标记待检查"
HEADER_START_RE = re.compile(r"^\s*\{\{\s*header/裁判文书\b", re.IGNORECASE)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Promote overwrite-reverted pages whose only difference is fixed redaction markup."
    )
    parser.add_argument(
        "--category",
    ...
    parser.add_argument("--skip", type=int, default=0, help="Number of category members to skip.")
    parser.add_argument("--max", type=int, default=None, help="Maximum category members to process.")
    parser.add_argument("--batch-size", type=int, default=20, help="Category/revision batch size.")
    parser.add_argument("--quiet", action="store_true", help="Only print periodic progress and failures.")
    parser.add_argument("--progress-every", type=int, default=50, help="Progress interval in quiet mode.")
    parser.add_argument("--interval", type=float, default=1.0, help="Minimum seconds between edits.")
    parser.add_argument("--maxlag", type=int, default=5, help="MediaWiki maxlag value.")
    parser.add_argument("--dry-run", action="store_true", help="Log decisions without saving pages.")
    parser.add_argument(
        "--summary",
        default="",
        help="Edit summary to use when promoting revisions (default: empty).",
    )
    parser.add_argument(
        "--log-dir",
        type=Path,
        default=DEFAULT_LOG_DIR,
        help=f"Directory for JSONL logs (default: {DEFAULT_LOG_DIR})",
    )
    parser.add_argument(
        "--lock-path",
        type=Path,
        default=DEFAULT_LOCK_PATH,
        help=f"Single-run lock file path (default: {DEFAULT_LOCK_PATH})",
    )
    parser.add_argument(
        "--no-require-overwrite-pair",
        action="store_true",
        help="Do not require the latest two edit summaries to be the standard import/revert pair.",
    )
...
def build_log_paths(log_dir: Path) -> dict[str, Path]:
    ...
    return {
        "fixed": log_dir / f"redaction_overwrite_fixed_{timestamp}.jsonl",
        "skipped": log_dir / f"redaction_overwrite_skipped_{timestamp}.jsonl",
        "failed": log_dir / f"redaction_overwrite_failed_{timestamp}.jsonl",
    }
...
def normalize_redactions_for_save(text: str) -> str:
    """Normalize redaction markers, excluding the header title parameter."""
    ...
    for line in lines:
        ...
        if HEADER_START_RE.match(line):
            in_header = True
            result.append(normalize_redaction_markers(line))
            continue
        ...
        if in_header:
            match = HEADER_PARAM_RE.match(line)
...
def redaction_templates_to_crosses(text: str) -> str:
    return PRC_REDACT_TEMPLATE_RE.sub(lambda match: "×" * int(match.group(1)), text or "")
...
def rescue_canonical_text(text: str) -> str:
    text = remove_unchecked_category(text)
    text = normalize_redactions_for_save(text)
    text = canonicalize_redaction_markers(text)
    text = redaction_templates_to_crosses(text)
...
def decide(title: str, revisions: list[RevisionText], *, require_overwrite_pair: bool) -> Decision:
    ...
    if OS_REDACTION_RE.search(current_without_category) or OS_REDACTION_RE.search(import_revision.text):
        return Decision("skip", "contains_os_redaction")

    candidate = remove_unchecked_category(import_revision.text)
    fixed_candidate = remove_unchecked_category(normalize_redactions_for_save(candidate))

    if normalize_wikitext_for_comparison(current_without_category) == normalize_wikitext_for_comparison(fixed_candidate):
        return Decision("skip", "candidate_matches_current_after_category_removal")

    if rescue_canonical_text(current_without_category) != rescue_canonical_text(fixed_candidate):
        return Decision("skip", "non_redaction_difference")

    current_templates = redaction_template_count(canonicalize_redaction_markers(current_without_category))
    candidate_templates = redaction_template_count(canonicalize_redaction_markers(fixed_candidate))
    current_penalty = body_redaction_penalty(current_without_category)
    candidate_penalty = body_redaction_penalty(fixed_candidate)

    if candidate_templates <= current_templates and candidate_penalty >= current_penalty:
        return Decision("skip", "candidate_is_not_redaction_markup_improvement")

    return Decision("fix", "redaction_markup_only", fixed_candidate.rstrip() + "\n")
...
def main() -> int:
    ...
    console.print("=" * 72)
    console.print("[bold cyan]Redaction Overwrite Revision Promoter[/bold cyan]")
    console.print("=" * 72)
            ...
            for title in raw_batch:
                ...
                try:
                    ...
                    if args.dry_run:
                        ...
                        if not args.quiet:
                            console.print("  [green]would fix[/green]")
                        continue
                ...
                if args.quiet and counts["processed"] % args.progress_every == 0:
                    console.print(
                        ...
                        f"fixed {counts['fixed']}; skipped {counts['skipped']}; failed {counts['failed']}"
                    )
    finally:
    ...
    console.print("=" * 72)
    console.print(
        f"Processed {counts['processed']}; fixed {counts['fixed']}; "
